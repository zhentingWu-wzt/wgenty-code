...
`Repl | Query | Config | Mcp | Plugin | Memory | Voice | Init | Update | Services | Agent | MagicDocs | TeamSync | Skills | Sandbox | StressTest | Daemon | Subagent`
...
- **Subagent**: 离线只读查询子代理 transcript（直读 SQLite，不启动 agent）
  - `list [--session <id>] [--status <s>] [--limit <n>]` — 倒序表格（id/label/status/root-cause/duration/rounds/started_at）
  - `trace <id> [--format call_tree|error_timeline|html|chrome] [--raw]` — 渲染单次 trace；`--raw` 打印诊断 JSON；未知 id 退出码 2
  - `health [--period 1h|24h|7d|30d|all] [--session <id>]` — 成功率 + 按 `FailureRootCause` 分组的失败模式
...
| `agent.rlm.delegate_tool` | bool | true | `delegate` 工具是否注册并可见 |
| `agent.rlm.auto_routing` | bool | true | `task` 工具自动路由复杂任务到 RLM 管道 |
| `agent.rlm.retry_enabled` | bool | true | RLM 子任务失败后是否触发增量 replan |
| `agent.rlm.max_replan_cycles` | usize | 2 | 最大 replan 循环次数（0=禁用反馈循环） |
| `agent.rlm.jaccard_threshold` | f64 | 0.8 | replan 替换提示词 Jaccard 去重阈值（0.0–1.0） |
...
| `prompt.debug_dump_reminder` | bool | false | 每轮把 user 消息的 `<system-reminder>` dump 到 `<project>/.wgenty-code/debug/reminders/`；也可用 env `WGENTY_DEBUG_REMINDER=1` |
...
静态指令走 **system cascade**（会话级缓存，不进每轮 user history）：

- `~/.wgenty-code/WGENTY.md` — 用户级全局指令（Layer 7）
- `~/.wgenty-code/rules/*.md` — 用户级规则（Layer 8，按文件名字母序）
- 项目根 `WGENTY.md` / `AGENTS.md` — Layer 9 / 10

每轮 user 消息上的 `<system-reminder>` **仅承载 hook 注入**（`UserPromptSubmit` → `InjectContext`）。无 hook 时不 prepend reminder。

调试：`prompt.debug_dump_reminder=true` 或 `WGENTY_DEBUG_REMINDER=1` 时，每轮 dump 到 `<project>/.wgenty-code/debug/reminders/`。
