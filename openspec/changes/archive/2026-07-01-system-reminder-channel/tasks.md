...
- [x] 2.1 在 `src/prompts/mod.rs` 新增私有常量 `REMINDER_PREAMBLE_OPENING` 和 `REMINDER_PREAMBLE_CLOSING`，精确复刻 Claude Code 措辞（含 `# wgentyMd` 与闭合 preamble 6 空格缩进）
...
- [x] 3.4 集成测：模拟 user 输入，断言第一轮 user message content 头部包含 `<system-reminder>` 块
- [x] 3.5 集成测：连续两轮 user 输入，第二轮 user message 再次包含 reminder（per-turn 验证）
