...
- [x] 6.1 验收：断线按 after=seq 续传；缓冲淘汰/Lagged 收到失步信号并正确回退全量恢复
- [x] 6.2 验收：todos/模式/模型变更推送到达全部订阅者；背景结果多端可见且离线可查
- [x] 6.3 验收：重复审批应答 409；并发会话写冲突 409；多会话审批隔离
- [x] 6.4 验收：两个 UI 经发现文件复用同一 daemon；失效发现文件不误连（人工验收完成 2026-08-08；daemon 侧端到端 + TUI 真机验证通过）
- [x] 6.5 TUI 双模式回归 + 现有 daemon/session 测试套件通过（测试套件已通过；TUI 双模式手动回归人工验收完成 2026-08-08）
