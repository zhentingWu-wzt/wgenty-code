...
daemon 与 TUI 每轮的跨会话记忆召回结果（TF-IDF top-N）必须通过 user-message `<system-reminder>` 通道注入到最后一条用户消息，不得写入 system 级消息。system 级联（含固定集合的 global memories Layer 5c）在会话配置不变的前提下跨 turn 逐字节稳定。
...
OpenAI 兼容响应的 `usage.prompt_tokens_details.cached_tokens`（及 Anthropic 的 `cache_read_input_tokens`）必须解析并贯通到 TurnContext 的 usage 数据；web Inspector Tokens 面板展示 cached 与 prompt 的比值。字段缺失时展示为 null，不得猜测。
...
发往 OpenAI 兼容端点的请求中，历史 assistant 消息不得携带 `reasoning_content`，DeepSeek provider 除外（保留 echo-back）。磁盘会话持久化与本地回放不受影响。
