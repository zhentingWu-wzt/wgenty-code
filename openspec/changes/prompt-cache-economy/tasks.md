...
- [x] 2.1 `src/api/types.rs`：OpenAI usage 增加 `prompt_tokens_details.cached_tokens`（serde default）；Anthropic 转换层映射 `cache_read_input_tokens` 到统一字段
- [x] 2.2 token_counter 增加 `last_cached_tokens`；`run_loop.rs` TurnContext usage JSON 增加 `cached_tokens` 字段
- [x] 2.3 web：`sessionStore.ts` `TurnContextUsage` 类型 + `InspectorPanel.tsx` TokensTab 显示 `cached / prompt (xx%)`（null 时显示 "—"）；聊天 StatusBar context 条旁新增 cache 命中徽标（`cache 80%`，null 时隐藏）
- [x] 2.4 测试：Rust 侧 usage 解析（含字段缺失缺省）+ web Inspector 显示（有值/null 两态）
- [x] 2.5 验证：`cargo test --lib api`、`cd web && npm test`
