...
impl SubagentHealthAnalyzer {
    ...
    pub fn compute_health(
        ...
        let filtered: Vec<&SubagentTranscriptHeader> = if let Some(cutoff_ms) = cutoff {
            transcripts
                .iter()
                .filter(|t| t.started_at >= cutoff_ms)
                .collect()
        } else {
    ...
    pub fn compute_from_headers(
        ...
        if total == 0 {
            return Ok(SubagentHealth {
                ...
                recommendations: vec![
                    "No subagent data yet. Run some subagent tasks to populate the dashboard."
                        .to_string(),
                ],
            });
        }

        let completed = transcripts
            .iter()
            .filter(|t| t.status == "completed")
            .count();
        let failed = transcripts.iter().filter(|t| t.status == "failed").count();
        let cancelled = transcripts
            .iter()
            .filter(|t| t.status == "cancelled")
            .count();
        let success_rate = completed as f64 / total.max(1) as f64;
        let failure_rate = (failed + cancelled) as f64 / total.max(1) as f64;
        let avg_rounds = transcripts
            .iter()
            .map(|t| t.actual_rounds as f64)
            .sum::<f64>()
            / total.max(1) as f64;
        let avg_tokens = transcripts
            .iter()
            .map(|t| t.total_tokens as f64)
            .sum::<f64>()
            / total.max(1) as f64;
        let avg_duration_ms = transcripts
            .iter()
        ...
        let mut failure_modes: Vec<FailureModeStats> = mode_counts
            ...
            .collect();
        failure_modes.sort_by_key(|b| std::cmp::Reverse(b.count));
        ...
        let mut recommendations =
            gen_recs(success_rate, avg_rounds, avg_tokens, &failure_modes, total);
        recommendations.push(format!(
            "Total {} subagent runs analyzed across period {:?}",
            total, period
        ));
    ...
    pub fn print_health_report(health: &SubagentHealth) {
        ...
        println!(
            "│ ❌ Failed:          {:>6}  ({:.1}%)",
            health.failed,
            (health.failed as f64 / health.total_runs.max(1) as f64) * 100.0
        );
        println!(
            "│ 🚫 Cancelled:       {:>6}  ({:.1}%)",
            health.cancelled,
            (health.cancelled as f64 / health.total_runs.max(1) as f64) * 100.0
        );
...
fn score(
    ...
    avg_rounds: f64,
    avg_tokens: f64,
    failure_modes: &[FailureModeStats],
) -> f64 {
    let base = success_rate * 70.0;
    let rounds_score = if avg_rounds > 25.0 {
        0.0
    } else if avg_rounds > 15.0 {
        5.0
    } else if avg_rounds > 8.0 {
        10.0
    } else {
        15.0
    };
    let token_score = if avg_tokens > 50_000.0 {
        0.0
    } else if avg_tokens > 20_000.0 {
        3.0
    } else if avg_tokens > 8_000.0 {
        7.0
    } else {
        10.0
    };
    let critical: f64 = failure_modes
        .iter()
        .filter(|fm| fm.severity == "Critical")
        .map(|fm| fm.count as f64)
        .sum();
    let total_f: f64 = failure_modes.iter().map(|fm| fm.count as f64).sum();
    let penalty = if total_f > 0.0 {
        (critical / total_f) * 15.0
    } else {
        0.0
    };
    ...
    match mode {
        FailureMode::Timeout => {
            if count >= 3 {
                ...
                "Occasional timeouts — monitor API latency".into()
            }
        }
        FailureMode::TokenBudgetExceeded => "Increase token_budget_k in settings".into(),
        FailureMode::StuckLoop => format!(
            "Stuck detection in {} runs — consider more specific prompts",
            count
        ),
        FailureMode::ParseError => "JSON parse errors — consider using a more capable model".into(),
        FailureMode::MaxRoundsExceeded => {
...
fn gen_recs(
    ...
    if success_rate < 0.50 {
        r.push(format!(
            "🔴 CRITICAL: {}% success rate. Immediate investigation required.",
            (success_rate * 100.0) as u32
        ));
    } else if success_rate < 0.70 {
        r.push(format!(
            "🟠 WARNING: {}% success rate. Review failure patterns.",
            (success_rate * 100.0) as u32
        ));
    } else if success_rate < 0.90 {
        r.push(format!(
            "🟡 NOTICE: {}% success rate — monitoring recommended.",
            (success_rate * 100.0) as u32
        ));
    }
    if let Some(top) = failure_modes.first() {
        if top.percentage > 50.0 {
            r.push(format!(
                "Dominant failure: {} ({}%) — focus here first.",
                top.label, top.percentage as u32
            ));
        }
    }
    if avg_rounds > 20.0 {
        r.push(format!(
            "High avg rounds ({:.0}/100) — consider splitting complex tasks.",
            avg_rounds
        ));
    }
    if avg_tokens > 40_000.0 && total > 5 {
        r.push(format!(
            "High avg tokens ({:.0}/run) — if budget is set, raise it.",
            avg_tokens
        ));
    }
    if total < 10 {
        r.push("Low sample size — more runs needed for accurate metrics.".into());
    }
    r
}
...
mod tests {
    ...
    fn test_classify() {
        assert_eq!(
            FailureMode::classify("Subagent timed out after 600s"),
            FailureMode::Timeout
        );
        assert_eq!(
            FailureMode::classify("Token budget exceeded: limit 50k"),
            FailureMode::TokenBudgetExceeded
        );
        assert_eq!(
            FailureMode::classify("Stuck in loop: repeated 3+ times"),
            FailureMode::StuckLoop
        );
        assert_eq!(
            FailureMode::classify("3 consecutive irrecoverable JSON errors"),
            FailureMode::ParseError
        );
        assert_eq!(
            FailureMode::classify("Subagent exceeded maximum rounds"),
            FailureMode::MaxRoundsExceeded
        );
        assert_eq!(
    ...
    fn test_status() {
        ...
        assert_eq!(
            HealthStatus::from_success_rate(0.80),
            HealthStatus::Degraded
        );
        assert_eq!(
            HealthStatus::from_success_rate(0.60),
            HealthStatus::Unhealthy
        );
        assert_eq!(
            HealthStatus::from_success_rate(0.30),
    ...
    #[test]
    fn test_empty() {
        let a = SubagentHealthAnalyzer {
            store: std::sync::Arc::new(
                SubagentTranscriptStore::open(std::path::Path::new("/tmp/h_empty.db")).unwrap(),
            ),
        };
    ...
    fn test_mixed() {
        ...
        let a = SubagentHealthAnalyzer {
            store: std::sync::Arc::new(
                SubagentTranscriptStore::open(std::path::Path::new("/tmp/h_mixed.db")).unwrap(),
            ),
        };
        let h = a
            .compute_from_headers(&hdrs.iter().collect::<Vec<_>>(), HealthPeriod::AllTime)
            .unwrap();
