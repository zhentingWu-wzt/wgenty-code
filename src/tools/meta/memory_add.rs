//! Memory Add Tool
//!
//! Lets the agent proactively write a memory entry to persistent storage
//! via `MemoryManager::add_memory()`, without waiting for context compaction.
//! Supports scope selection (project/global), memory type, tags, importance,
//! and retrieval mode (auto / ondemand).

use crate::context::{MemoryEntry, MemoryManager, MemoryOrigin, MemoryType, RetrievalMode};
use crate::tools::{Tool, ToolError, ToolOutput};
use async_trait::async_trait;
use std::sync::Arc;

pub struct MemoryAddTool {
    memory: Arc<MemoryManager>,
}

impl MemoryAddTool {
    pub fn new(memory: Arc<MemoryManager>) -> Self {
        Self { memory }
    }

    fn parse_memory_type(s: &str) -> Result<MemoryType, ToolError> {
        match s {
            "Knowledge" => Ok(MemoryType::Knowledge),
            "Preference" => Ok(MemoryType::Preference),
            "Session" => Ok(MemoryType::Session),
            "Conversation" => Ok(MemoryType::Conversation),
            "Task" => Ok(MemoryType::Task),
            "Error" => Ok(MemoryType::Error),
            "Insight" => Ok(MemoryType::Insight),
            "Decision" => Ok(MemoryType::Decision),
            _ => Err(ToolError {
                message: format!(
                    "Invalid memory_type '{}'. Must be one of: Knowledge, Preference, Session, Conversation, Task, Error, Insight, Decision",
                    s
                ),
                code: Some("invalid_memory_type".to_string()),
            }),
        }
    }

    fn parse_scope(s: &str) -> Result<MemoryOrigin, ToolError> {
        match s {
            "project" => Ok(MemoryOrigin::Project),
            "global" => Ok(MemoryOrigin::Global),
            _ => Err(ToolError {
                message: format!("Invalid scope '{}'. Must be 'project' or 'global'", s),
                code: Some("invalid_scope".to_string()),
            }),
        }
    }
}

#[async_trait]
impl Tool for MemoryAddTool {
    fn name(&self) -> &str {
        "memory_add"
    }

    fn description(&self) -> &str {
        "Write a memory entry to persistent storage. Use retrieval_mode='ondemand' for long documents (design docs, analysis) that should only appear on explicit search; use 'auto' (default) for short atomic facts injected every turn."
    }

    fn input_schema(&self) -> serde_json::Value {
        serde_json::json!({
            "type": "object",
            "properties": {
                "content": {
                    "type": "string",
                    "description": "The memory content to persist"
                },
                "memory_type": {
                    "type": "string",
                    "description": "Memory type category",
                    "enum": ["Knowledge", "Preference", "Session", "Conversation", "Task", "Error", "Insight", "Decision"],
                    "default": "Knowledge"
                },
                "scope": {
                    "type": "string",
                    "description": "Storage scope: 'project' for project-specific, 'global' for cross-project",
                    "enum": ["project", "global"],
                    "default": "project"
                },
                "tags": {
                    "type": "array",
                    "items": { "type": "string" },
                    "description": "Optional tags for categorization"
                },
                "importance": {
                    "type": "number",
                    "description": "Importance score (0.0-1.0)",
                    "default": 0.5,
                    "minimum": 0.0,
                    "maximum": 1.0
                },
                "retrieval_mode": {
                    "type": "string",
                    "description": "Injection strategy: 'auto' (default) = injected every turn via TF-IDF recall, for short atomic facts; 'ondemand' = only returned on explicit search, never auto-injected, for long documents (design docs, analysis, checklists)",
                    "enum": ["auto", "ondemand"],
                    "default": "auto"
                }
            },
            "required": ["content"]
        })
    }

    fn is_read_only(&self) -> bool {
        false
    }

    async fn execute(&self, input: serde_json::Value) -> Result<ToolOutput, ToolError> {
        let content = input["content"].as_str().ok_or_else(|| ToolError {
            message: "content is required".to_string(),
            code: Some("missing_content".to_string()),
        })?;

        let memory_type = match input["memory_type"].as_str() {
            Some(s) => Self::parse_memory_type(s)?,
            None => MemoryType::Knowledge,
        };

        let scope = match input["scope"].as_str() {
            Some(s) => Self::parse_scope(s)?,
            None => MemoryOrigin::Project,
        };

        let importance = input["importance"].as_f64().unwrap_or(0.5) as f32;

        let tags: Vec<String> = input["tags"]
            .as_array()
            .map(|arr| {
                arr.iter()
                    .filter_map(|v| v.as_str().map(String::from))
                    .collect()
            })
            .unwrap_or_default();

        let retrieval_mode = match input["retrieval_mode"].as_str() {
            Some("ondemand") => RetrievalMode::OnDemand,
            _ => RetrievalMode::Auto,
        };

        let mut entry = MemoryEntry::new(memory_type, content)
            .with_importance(importance)
            .with_tags(tags);
        entry.retrieval_mode = retrieval_mode;

        let result = self
            .memory
            .add_memory(entry, scope)
            .await
            .map_err(|e| ToolError {
                message: format!("Failed to add memory: {}", e),
                code: Some("memory_error".to_string()),
            })?;

        Ok(ToolOutput {
            output_type: "json".to_string(),
            content: serde_json::json!({
                "success": true,
                "memory_id": result.id,
                "merged": result.merged,
                "retrieval_mode": format!("{:?}", retrieval_mode),
            })
            .to_string(),
            metadata: std::collections::HashMap::new(),
        })
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use tempfile::tempdir;

    fn make_mm(tmp: &tempfile::TempDir) -> Arc<MemoryManager> {
        Arc::new(MemoryManager::new(tmp.path().to_path_buf()))
    }

    #[tokio::test]
    async fn is_read_only_returns_false() {
        let tmp = tempdir().unwrap();
        let tool = MemoryAddTool::new(make_mm(&tmp));
        assert!(!tool.is_read_only());
    }

    #[tokio::test]
    async fn creates_new_project_memory() {
        let tmp = tempdir().unwrap();
        let tool = MemoryAddTool::new(make_mm(&tmp));

        let input = serde_json::json!({
            "content": "The config file is at ~/.wgenty-code/settings.json",
            "scope": "project"
        });

        let result = tool.execute(input).await.unwrap();
        let parsed: serde_json::Value = serde_json::from_str(&result.content).unwrap();
        assert_eq!(parsed["success"], true);
        assert!(parsed["memory_id"].is_string());
    }

    #[tokio::test]
    async fn missing_content_returns_error() {
        let tmp = tempdir().unwrap();
        let tool = MemoryAddTool::new(make_mm(&tmp));

        let input = serde_json::json!({
            "memory_type": "Knowledge"
        });

        let err = tool.execute(input).await.unwrap_err();
        assert_eq!(err.code, Some("missing_content".to_string()));
    }

    #[tokio::test]
    async fn invalid_memory_type_returns_error() {
        let tmp = tempdir().unwrap();
        let tool = MemoryAddTool::new(make_mm(&tmp));

        let input = serde_json::json!({
            "content": "test",
            "memory_type": "InvalidType"
        });

        let err = tool.execute(input).await.unwrap_err();
        assert_eq!(err.code, Some("invalid_memory_type".to_string()));
    }

    #[tokio::test]
    async fn retrieval_mode_ondemand_is_stored() {
        let tmp = tempdir().unwrap();
        let mm = make_mm(&tmp);
        let tool = MemoryAddTool::new(mm.clone());

        let input = serde_json::json!({
            "content": "Design decision: use PostgreSQL for the event store",
            "retrieval_mode": "ondemand"
        });

        let result = tool.execute(input).await.unwrap();
        let parsed: serde_json::Value = serde_json::from_str(&result.content).unwrap();
        assert_eq!(parsed["success"], true);

        // Verify the memory is stored and searchable
        let found = mm.search_memories("PostgreSQL event store").await;
        assert!(
            !found.is_empty(),
            "memory with ondemand retrieval_mode should be searchable"
        );
        let stored = found.iter().find(|m| m.content.contains("PostgreSQL"));
        assert!(
            stored.is_some(),
            "memory content should match the stored entry"
        );
        assert_eq!(
            stored.unwrap().retrieval_mode,
            RetrievalMode::OnDemand,
            "retrieval_mode should be OnDemand when set to 'ondemand'"
        );
    }

    #[tokio::test]
    async fn retrieval_mode_defaults_to_auto() {
        let tmp = tempdir().unwrap();
        let mm = make_mm(&tmp);
        let tool = MemoryAddTool::new(mm.clone());

        // No retrieval_mode specified — should default to Auto
        let input = serde_json::json!({
            "content": "The build cache is cleared with `cargo clean`",
        });

        let result = tool.execute(input).await.unwrap();
        let parsed: serde_json::Value = serde_json::from_str(&result.content).unwrap();
        assert_eq!(parsed["success"], true);

        // Verify the memory is stored with default Auto retrieval_mode
        let found = mm.search_memories("build cache cargo clean").await;
        assert!(
            !found.is_empty(),
            "memory with default retrieval_mode should be searchable"
        );
        let stored = found.iter().find(|m| m.content.contains("build cache"));
        assert!(
            stored.is_some(),
            "memory content should match the stored entry"
        );
        assert_eq!(
            stored.unwrap().retrieval_mode,
            RetrievalMode::Auto,
            "default retrieval_mode should be Auto"
        );
    }
}
