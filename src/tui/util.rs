...
/// Return value for the reuse path: a shutdown sender whose signal is awaited
/// by a no-op task. The reused daemon outlives this TUI, so shutdown must
/// NOT stop it and must NOT remove its token file.
#[cfg(feature = "daemon")]
fn reused_daemon_handle(
    base_url: String,
) -> (
    String,
...
pub async fn start_daemon(
    ...
    if let Some(base_url) = try_reuse_running_daemon(DEFAULT_PORT, &working_dir).await {
        ...
        return Ok(reused_daemon_handle(base_url));
    }
