...
export function ReasoningBlock({ text, live }: { text: string; live: boolean }) {
  ...
  useEffect(() => {
    ...
    if (expanded && el && live && pinnedRef.current) el.scrollTop = el.scrollHeight;
  }, [text, live, expanded]);
  return (
    <div className="rounded-md border border-border bg-background text-[12px] text-muted-foreground">
      <button
        ...
        className="flex w-full items-center gap-1.5 px-2 py-1 text-left font-medium hover:text-foreground"
      >
