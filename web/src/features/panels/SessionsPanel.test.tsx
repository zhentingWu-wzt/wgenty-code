...
function resetStores() {
  ...
  useUiStore.setState({ rightPanel: "sessions" });
}
...
describe("SessionsPanel", () => {
  ...
  it("lists saved sessions and opens one losslessly (tool calls preserved)", async () => {
    ...
    await waitFor(() => {
      ...
      // Tool calls are preserved losslessly as standalone `role:"tool"` entries
      // carrying `toolExec`.
      const exec = msgs
        .flatMap((m) => (m.toolExec ? [m.toolExec] : []))
  ...
  it("opening a session closes the right panel", async () => {
    vi.stubGlobal("fetch", stubSessionsFetch());
    render(<SessionsPanel client={client} />);
    await userEvent.setup().click(await screen.findByText("saved one"));
...
describe("SessionsPanel archive view", () => {
  beforeEach(resetStores);
  afterEach(() => vi.unstubAllGlobals());
